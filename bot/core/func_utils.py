...
async def get_fsubs(uid, txtargs):
    ...
    for no, chat in enumerate(Var.FSUB_CHATS, start=1):
        try:
            ...
            sta = "Joined âœ…ï¸" if member.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.MEMBER] else "Not Joined âŒï¸"
            if sta == "Not Joined âŒï¸":
    ...
    if len(txtargs) > 1:
        btns.append([InlineKeyboardButton('ðŸ—‚ Get Files', url=f'https://t.me/{(await bot.get_me()).username}?start={txtargs[1]}')])
    return txt, btns
